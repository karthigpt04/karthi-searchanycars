...
interface ListingPayload {
  categoryId: number | null; listingCode: string; title: string; brand: string; model: string; variant: string;
  modelYear: number; registrationYear: number; vehicleType: string; bodyStyle: string; exteriorColor: string; interiorColor: string;
  listingPriceInr: number; negotiable: boolean; estimatedMarketValueInr: number | null;
  ownershipType: string; sellerType: string; registrationState: string; registrationCity: string;
  totalKmDriven: number; mileageKmpl: number | null; engineType: string; engineCapacityCc: number | null;
  powerBhp: number | null; transmissionType: string; fuelType: string; batteryCapacityKwh: number | null;
  overallConditionRating: number; serviceHistoryAvailable: boolean;
  airbagsCount: number; infotainmentScreenSize: string; locationCity: string; locationState: string;
  dealerRating: number | null; inspectionStatus: string; inspectionScore: number | null;
  listingStatus: string; featuredListing: boolean; isSplus: boolean; isNewCar: boolean; newCarType: string; promotionTier: string;
...
const brands = ['Maruti Suzuki','Hyundai','Tata','Honda','Kia','Mahindra','Toyota','Volkswagen','Skoda','Renault','Ford','MG','Nissan','BMW','Mercedes-Benz','Audi','Volvo','Jeep','Citroen','Porsche'];
...
export function AdminCarForm({ listingId }: { listingId?: number }) {
  ...
  const handleSubmit = async (e?: FormEvent) => {
    ...
    try {
      ...
      if (isEditing && listingId) {
        await api.updateListing(listingId, payload as unknown as Record<string, unknown>);
        setMessage('Listing updated successfully!');
      } else {
        await api.createListing(payload as unknown as Record<string, unknown>);
      ...
      <div className="adm-header">
        <div className="container">
          <div className="adm-header-row">
            <div>
              <Link href="/admin" className="adm-back-link">&larr; Back to Inventory</Link>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <Link href="/admin" className="btn btn-ghost" style={{ color: 'rgba(255,255,255,0.7)' }}>Cancel</Link>
      ...
      <form className="container" style={{ marginTop: '1.5rem', marginBottom: '3rem' }} onSubmit={handleSubmit}>
        <div className="adm-form-layout">
                ...
                <div className="adm-form-body">
                  ...
                  <div className="adm-field-row-3">
                    <div className="adm-field">
                      ...
                      <input className="adm-input" type="number" value={form.modelYear ?? ''} onChange={(e) => set('modelYear', e.target.value ? Number(e.target.value) : new Date().getFullYear())} min={2000} max={2030} />
                    </div>
                    <div className="adm-field">
                      <label className="adm-label">Registration Year</label>
                      <input className="adm-input" type="number" value={form.registrationYear ?? ''} onChange={(e) => set('registrationYear', e.target.value ? Number(e.target.value) : new Date().getFullYear())} min={2000} max={2030} />
                    </div>
                    <div className="adm-field">
                ...
                <div className="adm-form-body">
                  <div className="adm-field-row-3">
                    <div className="adm-field">
                      ...
                      <input className="adm-input" type="number" min={1} max={10} value={form.overallConditionRating ?? ''} onChange={(e) => set('overallConditionRating', e.target.value ? Number(e.target.value) : 8)} />
                    </div>
                    <div className="adm-field">
                  ...
                  <div className="adm-field-row-3">
                    ...
                    <div className="adm-field">
                      ...
                      <input className="adm-input" type="number" min={0} max={10} value={form.airbagsCount ?? ''} onChange={(e) => set('airbagsCount', e.target.value ? Number(e.target.value) : 2)} />
                    </div>
                    <div className="adm-field">
          ...
          <div className="adm-form-sidebar">
            ...
            {/* Quick Summary */}
            <div className="adm-form-card">
              <div className="adm-form-body" style={{ paddingTop: '1.25rem' }}>
