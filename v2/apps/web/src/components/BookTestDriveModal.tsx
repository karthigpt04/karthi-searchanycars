        ...
        {submitted ? (
          <div className="modal-body">
            <div className="form-success">
              ...
              <button className="btn btn-primary" onClick={onClose} style={{ marginTop: '1rem' }} type="button">Done</button>
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              ...
              {error && <p style={{ fontSize: '0.85rem', color: 'var(--error)', marginBottom: '0.75rem' }}>{error}</p>}
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.25rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', cursor: 'pointer', fontSize: '0.9rem' }}>
                    <input type="radio" name="location" value="home" checked={location === 'home'} onChange={() => setLocation('home')} /> Home Test Drive
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', cursor: 'pointer', fontSize: '0.9rem' }}>
                    <input type="radio" name="location" value="hub" checked={location === 'hub'} onChange={() => setLocation('hub')} /> Visit Hub
                  </label>
                </div>
              </div>
            </div>
