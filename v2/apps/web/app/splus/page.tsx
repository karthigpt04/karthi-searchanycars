          ...
          <div className="search-layout">
            ...
            <aside className={`filter-panel ${mobileFilterOpen ? 'filter-panel-open' : ''}`}>
              ...
              <div className="filter-section">
                ...
                {openSections.brand && <input className="filter-input" value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="e.g., BMW, Mercedes..." aria-label="Brand filter" />}
              </div>
              ...
              {/* Color */}
              <div className="filter-section">
                <button className="filter-section-title" onClick={() => toggleSection('color')} type="button" aria-expanded={openSections.color}>
                  Exterior Color <span className={`chevron ${openSections.color ? 'open' : ''}`}>&#9660;</span>
                </button>
                {openSections.color && (
                  <div className="filter-chips">
                    {colors.map((c) => <button key={c} className={`filter-chip ${color === c ? 'active' : ''}`} onClick={() => setColor(color === c ? '' : c)} type="button">{c}</button>)}
                  </div>
                )}
              </div>
              ...
              <div className="results-bar">
                ...
                {activeFilterCount > 0 && (
                  <div className="active-filters">
                    ...
                    {ownerType && <span className="active-filter-pill">{ownerType} Owner <button onClick={() => setOwnerType('')} type="button">&#10005;</button></span>}
                    {color && <span className="active-filter-pill">{color} <button onClick={() => setColor('')} type="button">&#10005;</button></span>}
