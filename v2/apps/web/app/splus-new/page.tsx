        ...
        <div className="container">
          ...
          <div className="search-layout">
            ...
            <aside className={`filter-panel ${mobileFilterOpen ? 'filter-panel-open' : ''}`}>
              ...
              {/* Car Type */}
              <div className="filter-section">
                <button className="filter-section-title" onClick={() => toggleSection('type')} type="button" aria-expanded={openSections.type}>
                  Car Type <span className={`chevron ${openSections.type ? 'open' : ''}`}>&#9660;</span>
                </button>
                {openSections.type && (
                  <div className="filter-chips">
                    {carTypes.map((t) => (
                      <button key={t.value} className={`filter-chip ${carType === t.value ? 'active' : ''}`} onClick={() => setCarType(carType === t.value ? '' : t.value)} type="button">{t.label}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="results-bar">
                ...
                {activeFilterCount > 0 && (
                  <div className="active-filters">
                    {carType && <span className="active-filter-pill">{carType} <button onClick={() => setCarType('')} type="button">&#10005;</button></span>}
